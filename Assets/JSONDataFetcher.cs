...
public class JSONDataFetcher : MonoBehaviour
{
    ...
    private string ConvertNodeIdToKey(string nodeId)
    {
        if (string.IsNullOrEmpty(nodeId))
            return null;

        // Пример: если nodeId приходит как "Channel_1_Axis_1 (X)_CurPos"
        // можно вернуть его как есть или преобразовать по правилам
        return nodeId.Trim();
    }
